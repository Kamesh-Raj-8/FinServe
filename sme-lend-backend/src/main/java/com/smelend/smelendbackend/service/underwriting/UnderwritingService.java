        ...
        if (app.getCreatedBy() != null) {
            ...
            if (_email != null) {
                switch (req.getDecision()) {
                    case APPROVE -> notificationService.notifyUnderwriterApproved(_email, _name, app.getApplicationId());
                    case REJECT  -> notificationService.notifyUnderwriterRejected(_email, _name, app.getApplicationId());
                    case RETURN  -> notificationService.notifyApplicationReturned(_email, _name, app.getApplicationId(),
                            req.getSummaryNote() != null ? req.getSummaryNote() : "Please re-verify KYC.");
                }
            }
        }

...
public class SmeService {
    ...
    public SmeResponse update(Long smeId, UpdateSmeRequest req) {
        ...
        sme.setRegistrationNo(req.getRegistrationNo());
    ...
    public List<SmeResponse> listMine() {
        AppUser me = currentUserService.getCurrentUser();
        String role = me.getRole().getRoleName().name();
        if (role.equals("AGENT") || role.equals("ADMIN") || role.equals("UNDERWRITER")) {
            return smeRepo.findAll().stream().map(this::toDto).toList();
        }

...
import { SmeResponse, PromoterResponse, BizType } from '../../../core/models';
...
export class SmeDetailComponent implements OnInit {
  ...
  pForm = this.fb.group({ promoterName: ['', Validators.required], mobile: ['', Validators.required], ownershipPct: [null as number|null, [Validators.required, Validators.min(0.01), Validators.max(100)]] });
  ...
  saveEdit() {
    ...
    this.api.updateSme(Number(this.smeId), { legalName: v.legalName!, tradeName: v.tradeName||undefined, businessType: v.businessType as BizType, industry: v.industry!, address: v.address!, gstNo: v.gstNo?.toUpperCase()||undefined }).subscribe({
      next: r => { this.savingSme.set(false); this.sme.set(r.data); this.showEditModal.set(false); this.flash('SME updated', 'success'); },
  ...
  addPromoter() {
    ...
    this.api.addPromoter(Number(this.smeId), { promoterName: v.promoterName!, mobile: v.mobile!, ownershipPct: v.ownershipPct! }).subscribe({
